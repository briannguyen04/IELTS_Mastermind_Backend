...
@Entity
@Table(
        name = "submission_topic_tag_accuracy",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_submission_topic_tag",
                        columnNames = {"user_practice_submission_id", "topic_tag"}
                )
        }
)
public class SubmissionTopicTagAccuracy {

    @Id
    @Column(name = "submission_topic_tag_accuracy_id", nullable = false, updatable = false)
